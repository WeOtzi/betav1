...
function renderApplicationsView() {
    ...
    let html = `
    <div style="display:grid; grid-template-columns: auto 1fr auto auto auto auto auto; gap:0; border:2px solid var(--fg,#0A0A0A);">
        <div style="background:var(--fg,#0A0A0A); color:var(--bg,#F2F0E9); padding:10px 16px; font-weight:900; font-size:0.75rem; text-transform:uppercase; font-family:'JetBrains Mono',monospace;">Fecha</div>
        <div style="background:var(--fg,#0A0A0A); color:var(--bg,#F2F0E9); padding:10px 16px; font-weight:900; font-size:0.75rem; text-transform:uppercase; font-family:'JetBrains Mono',monospace;">Idea</div>
        <div style="background:var(--fg,#0A0A0A); color:var(--bg,#F2F0E9); padding:10px 16px; font-weight:900; font-size:0.75rem; text-transform:uppercase; font-family:'JetBrains Mono',monospace;">Ciudad</div>
        <div style="background:var(--fg,#0A0A0A); color:var(--bg,#F2F0E9); padding:10px 16px; font-weight:900; font-size:0.75rem; text-transform:uppercase; font-family:'JetBrains Mono',monospace;">Zona</div>
        <div style="background:var(--fg,#0A0A0A); color:var(--bg,#F2F0E9); padding:10px 16px; font-weight:900; font-size:0.75rem; text-transform:uppercase; font-family:'JetBrains Mono',monospace;">Presupuesto</div>
        <div style="background:var(--fg,#0A0A0A); color:var(--bg,#F2F0E9); padding:10px 16px; font-weight:900; font-size:0.75rem; text-transform:uppercase; font-family:'JetBrains Mono',monospace;">Estado</div>
        <div style="background:var(--fg,#0A0A0A); color:var(--bg,#F2F0E9); padding:10px 16px; font-weight:900; font-size:0.75rem; text-transform:uppercase; font-family:'JetBrains Mono',monospace;">Mi Precio</div>
    `;

    myApplications.forEach(app => {
        const req = app.job_board_requests || {};
        const date = new Date(app.created_at).toLocaleDateString('es-ES', { day: '2-digit', month: '2-digit', year: '2-digit' });
        const idea = (req.tattoo_idea_description || '').substring(0, 40) + ((req.tattoo_idea_description || '').length > 40 ? '...' : '');
        const budget = req.client_budget_min && req.client_budget_max ? `$${req.client_budget_min}-$${req.client_budget_max}` : '-';
        const isAccepted = app.status === 'accepted';

        html += `
            <div style="padding:10px 16px; border-bottom:1px solid rgba(0,0,0,0.1); font-size:0.85rem; display:flex; align-items:center;">${date}</div>
            <div style="padding:10px 16px; border-bottom:1px solid rgba(0,0,0,0.1); font-size:0.85rem; display:flex; align-items:center;">
                <span style="font-weight:600;">${req.request_code || ''}</span>&nbsp;${idea}
            </div>
            <div style="padding:10px 16px; border-bottom:1px solid rgba(0,0,0,0.1); font-size:0.85rem; display:flex; align-items:center;">${req.client_city || '-'}</div>
            <div style="padding:10px 16px; border-bottom:1px solid rgba(0,0,0,0.1); font-size:0.85rem; display:flex; align-items:center;">${req.tattoo_body_part || '-'}</div>
            <div style="padding:10px 16px; border-bottom:1px solid rgba(0,0,0,0.1); font-size:0.85rem; display:flex; align-items:center;">${budget}</div>
            <div style="padding:10px 16px; border-bottom:1px solid rgba(0,0,0,0.1); display:flex; align-items:center;">
            ...
            <div style="padding:10px 16px; border-bottom:1px solid rgba(0,0,0,0.1); font-size:0.85rem; display:flex; align-items:center;">
                ${app.estimated_price || '-'}
                ${isAccepted && req.resulting_quote_id ? ' <a href="/my-quotations" style="margin-left:8px; color:var(--primary-blue,#1A4B8E); font-weight:700; font-size:0.75rem;">VER</a>' : ''}
            </div>
        `;
    });
